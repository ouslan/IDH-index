...
class DataProcess(DataPull):
    ...
    def __init__(
        ...
        super().__init__(saving_dir=saving_dir, database_file=database_file)

    def health_index(self) -> ibis.expr.types.relations.Table:
        """
        ...
        # ffill strategy for missing values
    ...
    def edu_index(self, folder_path: str = "data/raw/", debug: bool = False) -> None:
        ...
        None
        """
        edu_index = pd.DataFrame(
            [], columns=["year", "edu_index", "edu_index_adjusted"]
        )
        for file in os.listdir(folder_path):
            if file.startswith("data_ppr"):
                ...
                edu_sch["schooling"] = edu_sch["schooling"].apply(
                    lambda x: self.to_category(x)
                )
                ...
                coef, amean, gemetric, atkinson = self.adjust(
                    edu_sch["no_zero_schooling"]
                )
        ...
        edu_index["growth_rate_adjusted"] = (
            edu_index["edu_index_adjusted"].pct_change() * 100
        )
    ...
    def idh_index(self, debug: bool = False) -> None:
        ...
        df["index"] = (df["health_index"] * df["income_index"] * df["edu_index"]) ** (
            1 / 3
        )
        df["index_adjusted"] = (
            df["health_index_adjusted"]
            * df["income_index_adjusted"]
            * df["edu_index_adjusted"]
        ) ** (1 / 3)
